...
// E2E runs anonymous against local Supabase with no seeded transcript, so the
// review page shows its empty state. The task mutation/sync flows need a
// seeded transcript + Trello connection and are out of scope for the MVP E2E
// suite (see task 8.2).

test("review page shows heading and zero-task state for anonymous visitor", async ({
  page,
}) => {
